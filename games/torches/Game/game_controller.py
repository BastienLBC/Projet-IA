...
"""
from .gameModel.players import *
from .gameModel.game_model import *
...
def training(ai1, ai2, nb_games, nb_epsilon):
    ...
    for i in range(0, nb_games):
        if i % nb_epsilon == 0:
            if isinstance(ai1, ia_player): ai1.next_epsilon()
            if isinstance(ai2, ia_player): ai2.next_epsilon()

        training_game.play()
        if isinstance(ai1, ia_player): ai1.train()
        if isinstance(ai2, ia_player): ai2.train()
